...
class ThreeApp {
  ...
  static MATERIAL_PARAM = {
    shaderSourcePath: {
      vertex: './shaders/render.vert',
      fragment: './shaders/render.frag'
    }
  };
  ...
  init() {
    ...
    // パーティクルのセットアップ
    this.particleGeometry = new THREE.IcosahedronGeometry(
      ThreeApp.ICOSAHEDRON_PARAM.radius,
      ThreeApp.ICOSAHEDRON_PARAM.detail
    );
    this.particleMaterial = new THREE.ShaderMaterial({
      vertexShader: this.shaderSource.vertex,
      fragmentShader: this.shaderSource.fragment,
      transparent: true
    });
    this.particles = [];
  ...
  async load() {
    return new Promise(async (resolve, reject) => {
      this.shaderSource.vertex = await WebGLUtility.loadFile(ThreeApp.MATERIAL_PARAM.shaderSourcePath.vertex);
      this.shaderSource.fragment = await WebGLUtility.loadFile(ThreeApp.MATERIAL_PARAM.shaderSourcePath.fragment);
      resolve();
    });
  }
...
class Particle {
  ...
  constructor([x, y]) {
    ...
    // 位置の初期化
    this.x = x + 0.12 * (Math.random() - 0.5);
    this.y = y + 0.12 * (Math.random() - 0.5);
    this.z = 0;

    // スケールの初期化
    this.scale = 0.2 * Math.random();
    this.maxScale = Math.pow(Math.random(), 3.0); // 最大スケール
    this.deltaScale = 0.1 * 0.075 * Math.random(); // スケールの変化量

    // 削除フラグ
    this.toDelete = false;

    // 浮遊フラグ (6%の確率で浮遊)
    this.isFloating = Math.random() < 0.06;
  }
  ...
  grow() {
    ...
    if (this.scale >= this.maxScale) {
      ...
      this.deltaScale += 0.5; // 急激に大きくなるように設定
    }
